...
func InitRouter() *gin.Engine {
	...
	api.Use(middlewares.Logger())
	{
		...
		movie := api.Group("movie")
		{
			...
			// GET api/movie/noauth/moviepath | 看电影
			movie.GET("/noauth/movies/:name", service.DowFileWithOutAuth)
		}
		//需要jwt验证的接口
		auth := api.Group("")
		auth.Use(middlewares.TokenAuthorization())
		{
			...
			movies := auth.Group("movie")
			{
				...
				// GET api/movie/like/:id | 点赞
				movies.GET("like/:id", service.HandlerBindQuery(&service.LikeMovie{}))
